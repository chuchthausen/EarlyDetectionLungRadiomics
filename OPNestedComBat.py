...
# small modifications made by Claire Huchthausen on lines 71-80
import pandas as pd
...
def OPNestedComBat(dat, covars, batch_list, filepath, categorical_cols=None, continuous_cols=None, return_estimates=False):
    ...
    # CLAIRE HUCHTHAUSEN EDIT -- modified to handle len(batch_list)==1 case
    ...
    if len(batch_list) !=1:
        ...
        best_order_list = list(map(int, best_order[1:-1].split(',')[0]))
